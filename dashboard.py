...
def _next_observation(self):
    ...
    obs = np.array([
        current_data['price'] / 100000,
        current_data['volume'] / 1000000,
        np.clip(current_data.get('RSI_14', 50) / 100, 0, 1),
        current_data.get('MACD', 0) / 1000,
        current_data.get('MACD_Signal', 0) / 1000,
        max(0, self.balance / self.initial_balance),
        max(0, self.btc_held * current_data['price'] / self.initial_balance),
        np.clip(self.current_step / len(self.df), 0, 1),
