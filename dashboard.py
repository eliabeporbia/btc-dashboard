...
try:
    ...
    try:
        ...
        # Fallback para tensorflow.keras se tf_keras não estiver instalado
...
RL_ENV_CONFIG_PATH = os.path.join(MODEL_DIR, "rl_env_config.joblib") # Para guardar colunas e médias/std usadas
...
# --- Funções de Dados/Plotagem (mantidas) ---
...
def calculate_metrics(df): # Mantida
    ...
    metrics['Retorno Estratégia'] = df['strategy_cumulative'].iloc[-1] - 1; metrics['Retorno Buy & Hold'] = df['cumulative_return'].iloc[-1] - 1
    metrics['Vol Estratégia'] = returns.std() * np.sqrt(365); metrics['Vol Buy & Hold'] = buy_hold_returns.std() * np.sqrt(365)
    ...
    cum_returns_strat = df['strategy_cumulative']; peak_strat = cum_returns_strat.expanding(min_periods=1).max(); drawdown_strat = (cum_returns_strat - peak_strat) / peak_strat
    metrics['Max Drawdown Estratégia'] = drawdown_strat.min() if not drawdown_strat.empty else 0.0
    cum_returns_bh = df['cumulative_return']; peak_bh = cum_returns_bh.expanding(min_periods=1).max(); drawdown_bh = (cum_returns_bh - peak_bh) / peak_bh
...
def optimize_strategy_parameters(data, strategy_name, param_space): # Mantida
    if 'prices' not in data or data['prices'].empty: st.error("Dados ausentes otimização."); return None, -np.inf, None
    if not SKLEARN_AVAILABLE: st.error("SKlearn necessário otimização."); return None, -np.inf, None
    ...
    best_sharpe = -np.inf; best_params = None; best_results_df = None; total_combinations = len(param_combinations)
    progress_bar = st.progress(0); status_text = st.empty(); st.warning(f"Otimizando {total_combinations} combinações...")
